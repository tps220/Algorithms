...
//Thought process?
//How many blocks of size x can fit into a chain of length width (w)? 
//length 1 goes into a chain w times
//length 2 can overlap and go into it w - 1 times 
//length 3 can overlap and go into it w - 2 times
//keep on going until length w goes into it one time
//so its the sum from 1 to w, which is w * (w + 1) / 2
//Same logic follows for the height, so:
